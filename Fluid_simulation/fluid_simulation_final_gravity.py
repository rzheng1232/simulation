...
sigma = 20  # Controls the spread of the light, adjust this for smoother/diffuse light
...
# ]
pWIDTH = 200  # Replace with your desired width
pHEIGHT = 200  # Replace with your desired height

num_particles = 200
particle_pos = [
    (100 + random.random() * pWIDTH, 00 + random.random() * pHEIGHT) for _ in range(num_particles)
]
...
import math
import pygame
def mouse_particle_force(mouse_pos, particle_pos, m_radius):
    forces = []
    for p in particle_pos:
        ...
        if distance < m_radius:
            force_magnitude = smoothing_kernel(m_radius, distance) #(1 - distance / m_radius)  # Linear decay
...
def calculate_gradient(sample_point, particle_pos):
    ...
    curr_density = density_to_pressure(calculate_density(sample_point, particle_pos), 0.5, 0.8)
    dx = density_to_pressure(calculate_density((sample_point[0]+diff, sample_point[1]), particle_pos), 0.5,0.8) - curr_density
    dy = density_to_pressure(calculate_density((sample_point[0], sample_point[1]+diff), particle_pos), 0.5, 0.8) - curr_density
...
def update_particle_pos(particles, gradients, m_p, m_on, viscs):
    ...
    for i, p in enumerate(particles):
        ...
        # Update velocity and position
        particle_vs[i][0] += curr_gradient[0] + mouse_force[i][0] + curr_visc[0]
        particle_vs[i][1] += curr_gradient[1] + mouse_force[i][1] + curr_visc[1] +0.008
        new_p = (px + 100 * particle_vs[i][0], py + 100 * particle_vs[i][1])
        ...
        color_intensity = min(255, int(255 * (velocity_magnitude/0.2)**1))  # Multiply by 10 for better visibility
...
def draw_pressure_distribution(surface, pressure_data, max_pressure):
    # Loop through the pressure data and draw circles to represent pressure
    for (x, y), pressure in pressure_data.items():
        color_intensity = min(255, int(((pressure / (2*max_pressure) + max_pressure)**2) * 255))
        color = (color_intensity, 0, 255 - color_intensity)  # Gradient from blue (low) to red (high)
        pygame.draw.circle(surface, color, (x, y), 3)  # Small circle for pressure
...
while running:
    ...
    particle_pos = update_particle_pos(particle_pos,  particle_grads, mouse_pos, mouse_on, particle_viscs)
